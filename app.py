...
if biz_file:
    try:
        ...
        # Enforce column indexes explicitly matching your prompt request
...
with col4:
    st.markdown(f"<div class='kpi-card' style='border-top-color: {HEX_VIBRANT_BLUE};'><h4>Blended ROAS</h4><h2>{t_ro:.2f}x</h2><p>Return on Ad Spend</p></div>", unsafe_allow_html=True)
with col5:
    if has_tacos:
...
with tabs[3]:
    ...
    if best_port_row is not None:
        ...
        st.write(f"This efficiency gain is directly attributed to structural performance stability inside the `{p_name}` ecosystem. High-yield conversions inside `{top_camp}` drove volume without bloating cost per acquisition thresholds, securing scalable net-margin returns for the active period.")
